...
## 'Comparing First and Second Month VIX Futures'.
##
...
## Go long XIV (VXX) at today’s close when the front month VIX futures
## contract will close below (above) the second month. Hold until a
## change in position.
##
## For strategy details refer to
## http://volatilitymadesimple.com/backtest-comparing-first-and-second-month-vix-futures/
##  
...
strategy <- function(data)
  {
                                        ...
                                        # futures are in contago or backwardation
    ux1_ux2 <- mapply(FUN = function(front_month, back_month)
                      {
                        if(front_month <= back_month) ret_val =  1
                        if(front_month > back_month) ret_val = 2
                        return(ret_val)
                      }, front_month = data$UX1, back_month = data$UX2, SIMPLIFY = TRUE)
    
                                        # taking the froward and backward difference of the
                                        # above vector
    fdif_ux1_ux2 <- ux1_ux2[2:length(ux1_ux2)] - ux1_ux2[1:(length(ux1_ux2) - 1)]
    bdif_ux1_ux2 <- ux1_ux2[1:(length(ux1_ux2) - 1)] - ux1_ux2[2:length(ux1_ux2)]
    ...
    buy_xiv <- which(fdif_ux1_ux2 %in% -1); buy_xiv <- buy_xiv + 1
    sell_xiv <- which(bdif_ux1_ux2 %in% -1); sell_xiv <- sell_xiv + 1
    
                                        # determining the index positions when we want to buy
                                        # and sell VXX (this does not tell us the action to
                                        # be taken at the terminal points of the data-set)
    buy_vxx <- which(fdif_ux1_ux2 %in% 1); buy_vxx <- buy_vxx + 1
    sell_vxx <- which(bdif_ux1_ux2 %in% 1); sell_vxx <- sell_vxx + 1
    
                                        # determining the action to be taken at the terminal
                                        # points of the data set
    if(ux1_ux2[[1]] == 1) buy_xiv <- c(1, buy_xiv)
    if(ux1_ux2[[1]] == 2) buy_vxx <- c(1, buy_vxx)

    if(ux1_ux2[[length(ux1_ux2)]] == 1) sell_xiv <-c(sell_xiv, length(ux1_ux2))
    if(ux1_ux2[[length(ux1_ux2)]] == 2) sell_vxx <-c(sell_vxx, length(ux1_ux2))

                                        # dates when we want to buy and sell XIV
    buy_xiv_dates <- data$date[buy_xiv]
    sell_xiv_dates <- data$date[sell_xiv]  
                                        #  dates when we want to buy and sell back VXX
