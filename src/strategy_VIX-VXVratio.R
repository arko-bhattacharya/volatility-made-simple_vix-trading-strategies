...
strategy <- function(data)
  {
    ratio <- data$vix/data$vxv

                                        # xiv positions
    xiv_ind <- which(ratio <= 0.92)
    fdif_xiv_ind <- xiv_ind[2:length(xiv_ind)] - xiv_ind[1:(length(xiv_ind) - 1)]
    buy_ind_xiv <- xiv_ind[c(1, (which(fdif_xiv_ind != 1) + 1))]
    bdif_xiv_ind <- xiv_ind[1:(length(xiv_ind) - 1)] - xiv_ind[2:length(xiv_ind)]
    sell_ind_xiv <- xiv_ind[which(bdif_xiv_ind != -1)]; sell_ind_xiv <- sell_ind_xiv + 1
    sell_ind_xiv <- c(sell_ind_xiv, xiv_ind[length(xiv_ind)])

                                        # vxx positions
    vxx_ind <- which(ratio >= 1.08)
    fdif_vxx_ind <- vxx_ind[2:length(vxx_ind)] - vxx_ind[1:(length(vxx_ind) - 1)]
    buy_ind_vxx <- vxx_ind[c(1, (which(fdif_vxx_ind != 1) + 1))]
    bdif_vxx_ind <- vxx_ind[1:(length(vxx_ind) - 1)] - vxx_ind[2:length(vxx_ind)]
    sell_ind_vxx <- vxx_ind[which(bdif_vxx_ind != -1)]; sell_ind_vxx <- sell_ind_vxx + 1
    sell_ind_vxx <- c(sell_ind_vxx, vxx_ind[length(vxx_ind)])

                                        # dates when we want to buy and sell xiv
    buy_xiv_dates <- data$date[buy_ind_xiv]
    sell_xiv_dates <- data$date[sell_ind_xiv]
                                        #  dates when we want to buy and sell vxx
    buy_vxx_dates <- data$date[buy_ind_vxx]
    sell_vxx_dates <- data$date[sell_ind_vxx]
