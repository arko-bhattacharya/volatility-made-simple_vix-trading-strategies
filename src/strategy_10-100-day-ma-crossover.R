...
strategy <- function(data)
  {
    data_tmp <- zoo(x = data$xiv, order.by = data$date)
                                        ...
                                        # calculating the 10 day and 100 day MA
    avg_10 <- rollapply(data = data_tmp, 10, mean)
    avg_10 <- zoo(x = avg_10, order.by = data$date[10:nrow(data)])
    avg_100 <- rollapply(data = data_tmp, 100, mean)
    avg_100 <- zoo(x = avg_100, order.by = data$date[100:nrow(data)])
    avg <- merge.zoo(avg_10, avg_100, all = FALSE)

                                        # determining when 10 day MA > 100 day MA
    ...
    ret_val <- data.frame(ret_val = ret_val,
