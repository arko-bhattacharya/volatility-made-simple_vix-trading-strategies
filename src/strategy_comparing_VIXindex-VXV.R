...
## 'Comparing the VIX and VXV Indices'.
##
...
## VXV index, or go long VXX if it will close above. Hold until a
## change in position.
## 
## For strategy details refer to
## http://volatilitymadesimple.com/backtest-comparing-the-vix-and-vxv-indices/
##  
...
strategy <- function(data)
  {
                                        ...
                                        # index or vxv is greater
    vxv_vix <- mapply(FUN = function(vix_index, vxv)
                      {
                        if(vix_index <= vxv) ret_val =  1
                        if(vix_index > vxv) ret_val = 2
                        return(ret_val)
                      }, vix_index = data$vix, vxv = data$vxv, SIMPLIFY = TRUE)
    
                                        # taking the froward and backward difference of the
                                        # above vector
    fdif_vxv_vix <- vxv_vix[2:length(vxv_vix)] - vxv_vix[1:(length(vxv_vix) - 1)]
    bdif_vxv_vix <- vxv_vix[1:(length(vxv_vix) - 1)] - vxv_vix[2:length(vxv_vix)]
    ...
    buy_xiv <- which(fdif_vxv_vix %in% -1); buy_xiv <- buy_xiv + 1
    sell_xiv <- which(bdif_vxv_vix %in% -1); sell_xiv <- sell_xiv + 1
    
                                        # determining the index positions when we want to buy
                                        # and sell vxx (this does not tell us the action to
                                        # be taken at the terminal points of the data-set)
    buy_vxx <- which(fdif_vxv_vix %in% 1); buy_vxx <- buy_vxx + 1
    sell_vxx <- which(bdif_vxv_vix %in% 1); sell_vxx <- sell_vxx + 1
    
                                        # determining the action to be taken at the terminal
                                        # points of the data set
    if(vxv_vix[[1]] == 1) buy_xiv <- c(1, buy_xiv)
    if(vxv_vix[[1]] == 2) buy_vxx <- c(1, buy_vxx)

    if(vxv_vix[[length(vxv_vix)]] == 1) sell_xiv <- c(sell_xiv, length(vxv_vix))
    if(vxv_vix[[length(vxv_vix)]] == 2) sell_vxx <- c(sell_vxx, length(vxv_vix))
