...
strategy <- function(data)
  {
                                        ...
                                        # index or front month vix futures is greater
    ivix_ux1 <- mapply(FUN = function(front_month, vix_index)
                      {
                        if(vix_index <= front_month) ret_val =  1
                        if(vix_index > front_month) ret_val = 2
                        return(ret_val)
                      }, front_month = data$UX1, vix_index = data$vix, SIMPLIFY = TRUE)
    
                                        # taking the froward and backward difference of the
                                        # above vector
    fdif_ivix_ux1 <- ivix_ux1[2:length(ivix_ux1)] - ivix_ux1[1:(length(ivix_ux1) - 1)]
    bdif_ivix_ux1 <- ivix_ux1[1:(length(ivix_ux1) - 1)] - ivix_ux1[2:length(ivix_ux1)]
    ...
    buy_xiv <- which(fdif_ivix_ux1 %in% -1); buy_xiv <- buy_xiv + 1
    sell_xiv <- which(bdif_ivix_ux1 %in% -1); sell_xiv <- sell_xiv + 1
    
                                        # determining the index positions when we want to buy
                                        # and sell vxx (this does not tell us the action to
                                        # be taken at the terminal points of the data-set)
    buy_vxx <- which(fdif_ivix_ux1 %in% 1); buy_vxx <- buy_vxx + 1
    sell_vxx <- which(bdif_ivix_ux1 %in% 1); sell_vxx <- sell_vxx + 1
    
                                        # determining the action to be taken at the terminal
                                        # points of the data set
    if(ivix_ux1[[1]] == 1) buy_xiv <- c(1, buy_xiv)
    if(ivix_ux1[[1]] == 2) buy_vxx <- c(1, buy_vxx)

    if(ivix_ux1[[length(ivix_ux1)]] == 1) sell_xiv <- c(sell_xiv, length(ivix_ux1))
    if(ivix_ux1[[length(ivix_ux1)]] == 2) sell_vxx <- c(sell_vxx, length(ivix_ux1))
